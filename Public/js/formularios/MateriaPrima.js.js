...
function fLlenarCampos(Datos)
{
	...
	if ( Datos.analisisext == '1' )	{
		document.getElementById('radAnalisisExtSi').checked = true;
	}
	else	{
		document.getElementById('radAnalisisExtNo').checked = true;
	}
	if ( Datos.analisisint == '1' )	{
		document.getElementById('radAnalisisIntSi').checked = true;
	}
	else	{
		document.getElementById('radAnalisisIntNo').checked = true;
	}
...
function fAjax(Operacion)
{
	...
	$.ajax(
	{
	  cache: false, type: 'POST', dataType: 'json', url:'../Controladores/MateriaPrima.con.php',
...
function fTablaAjax()
{
	...
	xmlhttp.open('POST','../Modelos/Scripts/MateriaPrima.ajax.php',true);
