...
function fAjax(Operacion)
{
	...
	$.ajax(
	{
	  cache: false, type: 'POST', dataType: 'json', url:'../Controladores/SiloMP.con.php',
...
{
	if($('#txtNombre').val() == Datos.nombre && $('#cmbIdMateriaPrima').val() == Datos.idmateriaprima && $('#txtCapacidadMax').val() == Datos.capacidadmax && $('#txtDescripcion').val() == Datos.descripcion )	{
		return 0;
	}
	else {
		return 1;
...
function fTablaAjax()
{
	...
	xmlhttp.open('POST','../Modelos/Scripts/SiloMP.ajax.php',true);
