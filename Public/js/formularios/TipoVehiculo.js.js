...
{
	$('#txtIdTipoVehiculo').val(Datos.idtipovehiculo);
	$('#txtNombre').val(Datos.nombre.replace(/\+/g," "));
	$('#txtDescripcion').val(Datos.descripcion.replace(/\+/g," "));
	if ( Datos.remolque == '1' )	{
		document.getElementById('radRemolqueSi').checked = true;
	}
	else	{
		document.getElementById('radRemolqueNo').checked = true;
	}
	if ( Datos.recepcionmp == '1' )	{
		document.getElementById('radRecepcionMPSi').checked = true;
	}
	else	{
		document.getElementById('radRecepcionMPNo').checked = true;
	}
	if ( Datos.despachopt == '1' )	{
		document.getElementById('radDespachoPTSi').checked = true;
	}
	else	{
		document.getElementById('radDespachoPTNo').checked = true;
	}
...
function fAjax(Operacion)
{
	...
	$.ajax(
	{
	  cache: false, type: 'POST', dataType: 'json', url:'../Controladores/TipoVehiculo.con.php',
...
function fTablaAjax()
{
	...
	xmlhttp.open('POST','../Modelos/Scripts/TipoVehiculo.ajax.php',true);
