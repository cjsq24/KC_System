...
function fVerificarNombreDpto()
{
	if ( ( $('#txtOperacion').val() == 'registrar' && $('#txtNombre').val().length != '' && $('#cmbIdDepartamento').val() != '' ) )	{
		fAjax('verificar_nombre_dpto');
	}
	else if ( $('#txtOperacion').val() == 'modificar' && $('#txtNombre').val().length != Datos.nombre.replace(/\+/g," ") && $('#cmbIdDepartamento').val() != Datos.iddepartamento )	{
		fAjax('verificar_nombre_dpto');
	}
}
...
function fAjax(Operacion)
{
	...
	$.ajax(
	{
	  cache: false, type: 'POST', dataType: 'json', url:'../Controladores/Cargo.con.php',
	  ...
	  success: function(response)
	  {
	      ...
	      swal('!ERROR!', response.Mensaje , 'error');
			...
			else if(response.Resultado == 'existe' && Operacion == 'verificar_nombre_dpto')
			{
	      swal({ title: '¡ALERTA!', text: response.Mensaje, type: 'info', showCancelButton: true, cancelButtonText: 'NO', confirmButtonColor: '#DD6B55', confirmButtonText: 'SI', closeOnConfirm: true },function()	
	      {
	      	...
	      	$('#cmbIdDepartamentoBusq').val($('#cmbIdDepartamento').val());
	      	$('#cmbEstatus').val(''); 
	      	fBuscar();
	      	fOcultarFormulario();
...
{
	if($('#txtNombre').val() == Datos.nombre.replace(/\+/g," ") && $('#txtDescripcion').val() == Datos.descripcion.replace(/\+/g," ") && $('#cmbIdDepartamento').val() == Datos.iddepartamento )	{
		return 0;
	}
	else {
		return 1;
...
function fTablaAjax()
{
	...
	xmlhttp.open('POST','../Modelos/Scripts/Cargo.ajax.php',true);
	xmlhttp.setRequestHeader('Content-type','application/x-www-form-urlencoded');
	/*DATOS A ENVIAR AL SCRIPT DE LA TABLA*/
	xmlhttp.send('Campo=' + Campo + '&IdDepartamento=' + $('#cmbIdDepartamentoBusq').val() + '&Pagina=' + Pagina + '&TamanoPagina=' + TamanoPagina + '&VentPadre=' + VentPadre + '&Estatus=' + Estatus + '&IdMotivoCambioE=' + IdMotivoCambioE);
}
