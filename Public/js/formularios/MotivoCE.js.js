...
{
	$('#txtIdMotivoCE').val(Datos.idmotivoce);
	$('#txtNombre').val(Datos.nombre.replace(/\+/g," "));
	$('#cmbTipoMotivoCE').val(Datos.tipomotivo);
	$('#txtDescripcion').val(Datos.descripcion.replace(/\+/g," "));
	$('#txtEstatus').val(Datos.estatus);
}

function f(Operacion,IdMotivoCE,Nombre,TipoMotivoCE,Descripcion,SegundoApellido,Estatus)
{
	Operacion=Operacion;
	if($('#VentPadre').val()=='si')
	{
		opener.document.Form.txtIdMotivoCE.value=IdMotivoCE;
		opener.document.Form.txtNombreMotivoCE.value=Cedula;
		opener.document.Form.txtNombresMotivoCE.value=Nombre+' '+Descripcion;
		window.close();
	}
}
...
function fAjax(Operacion)
{
	...
	$.ajax({
	  cache: false, type: 'POST', dataType: 'json', url:'../Controladores/MotivoCE.con.php',
...
{
	if( $('#txtNombre').val() == Datos.nombre.replace(/\+/g," ") && $('#cmbTipoMotivoCE').val() == Datos.tipomotivo && $('#txtDescripcion').val() == Datos.descripcion.replace(/\+/g," ") )	{
		return 0;
	}
	else {
		return 1;
...
{
	var xmlhttp;
	var TipoMotivo = $('#cmbTipoMotivo').val();
	...
	xmlhttp.open('POST','../Modelos/Scripts/MotivoCE.ajax.php',true);
	xmlhttp.setRequestHeader('Content-type','application/x-www-form-urlencoded');
	/*DATOS A ENVIAR AL SCRIPT DE LA TABLA*/
	xmlhttp.send('TipoMotivo='+TipoMotivo+'&Campo='+Campo+'&Pagina='+Pagina+'&TamanoPagina='+TamanoPagina+'&VentPadre='+VentPadre+'&Estatus='+Estatus + '&IdMotivoCambioE=' + IdMotivoCambioE);
}
