...
{
	$('#txtIdListaAnalisisMP').val(Datos.idlistaanalisismp);
	$('#txtNombre').val(Datos.nombre.replace(/\+/g," "));
	$('#txtDescripcion').val(Datos.descripcion.replace(/\+/g," "));
	$('#cmbIdTipoInput').val(Datos.idtipoinput);
	$('#cmbIdUnidadMedida').val(Datos.idunidadmedida);
	$('#txtFuncion').val(Datos.funcion);
...
function fVerificarNombre()
{
	...
	else if ( $('#txtOperacion').val() == 'modificar' && $('#txtNombre').val().length != Datos.nombre.replace(/\+/g," ") )	{
		fAjax('verificar_nombre');
	}
}
...
function fAjax(Operacion)
{
	...
	$.ajax(
	{
	  cache: false, type: 'POST', dataType: 'json', url:'../Controladores/ListaAnalisisMP.con.php',
...
{
	if($('#txtNombre').val() == Datos.nombre.replace(/\+/g," ") && $('#txtDescripcion').val() == Datos.descripcion.replace(/\+/g," ") && $('#cmbIdTipoInput').val() == Datos.idtipoinput && $('#cmbIdUnidadMedida').val() == Datos.idunidadmedida && $('#txtFuncion').val() == Datos.funcion )	{
		return 0;
	}
	else {
		return 1;
...
function fTablaAjax()
{
	...
	var IdTipoInput = $('#cmbIdTipoInputBusq').val();
	...
	xmlhttp.open('POST','../Modelos/Scripts/ListaAnalisisMP.ajax.php',true);
	xmlhttp.setRequestHeader('Content-type','application/x-www-form-urlencoded');
	/*DATOS A ENVIAR AL SCRIPT DE LA TABLA*/
	xmlhttp.send('Campo=' + Campo + '&IdTipoInput=' + IdTipoInput + '&IdUnidadMedida=' + IdUnidadMedida + '&Pagina=' + Pagina + '&TamanoPagina=' + TamanoPagina + '&VentPadre=' + VentPadre + '&Estatus=' + Estatus + '&IdMotivoCambioE=' + IdMotivoCambioE);
}
