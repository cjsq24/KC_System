...
function fEnviarDatos()
{
	...
	{
		opener.$('#txtIdCliente').val(Datos.idcliente);
		opener.$('#txtRifCli').val( Datos.identificador + '-' + Datos.cedula + '-' + Datos.digitorif );
		opener.$('#txtRazonSocialCli').val( Datos.razonsocial.replace(/\+/g," ") );
		window.close();
	}
}
...
function fAjax(Operacion)
{
	...
	$.ajax(
	{
	  cache: false, type: 'POST', dataType: 'json', url:'../Controladores/Cliente.con.php',data: str + '&Operacion=' + Operacion,
...
function fTablaAjax()
{
	...
	xmlhttp.open('POST','../Modelos/Scripts/Cliente.ajax.php',true);
